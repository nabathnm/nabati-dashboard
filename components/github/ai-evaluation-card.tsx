...
export function AiEvaluationCard({ evaluation }: AiEvaluationCardProps) {
  return (
    <div className="bg-primary rounded-3xl p-6 text-primary-foreground shadow-xl relative overflow-hidden">
      <div className="absolute right-0 top-0 w-64 h-64 bg-indigo-500/10 blur-3xl rounded-full translate-x-1/3 -translate-y-1/3 pointer-events-none" />
      <div className="absolute left-0 bottom-0 w-64 h-64 bg-emerald-500/10 blur-3xl rounded-full -translate-x-1/3 translate-y-1/3 pointer-events-none" />
      ...
      <div className="flex items-center gap-3 mb-6 relative z-10">
        ...
        <div>
          ...
          <p className="text-xs text-indigo-200/70">Generated based on your GitHub activity</p>
        </div>
      </div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {evaluation.portfolio_review.map((point, idx) => (
              <li key={idx} className="flex gap-3 text-sm text-slate-300 bg-white/5 backdrop-blur-sm border border-white/5 rounded-xl p-3">
